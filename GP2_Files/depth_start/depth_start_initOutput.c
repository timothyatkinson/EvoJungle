...
#include "depth_start.h"

static bool match_n0(Morphism *morphism);

bool matchdepth_start_initOutput(Morphism *morphism)
{
   if(1 > depth_start_host->number_of_nodes || 0 > depth_start_host->number_of_edges) return false;
   if(match_n0(morphism)) return true;
   else
   {
      initialiseMorphism(morphism, depth_start_host);
      return false;
   }
}
...
{
   int host_index;
   for(host_index = 0; host_index < depth_start_host->nodes.size; host_index++)
   {
      Node *host_node = getNode(depth_start_host, host_index);
...
void applydepth_start_initOutput(Morphism *morphism, bool record_changes)
{
   ...
   HostLabel label_n0 = getNodeLabel(depth_start_host, host_node_index);
   HostLabel label;
   ...
   else
   {
      ...
      relabelNode(depth_start_host, host_node_index, label);
   }
   /* Reset the morphism. */
   initialiseMorphism(morphism, depth_start_host);
}
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < depth_start_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "depth_start_initOutput", depth_start_host);
