...
bool matchhorizontal_makeBlueRed(Morphism *morphism)
{
   if(2 > horizontal_host->number_of_nodes || 1 > horizontal_host->number_of_edges) return false;
...
void applyhorizontal_makeBlueRed(Morphism *morphism, bool record_changes)
{
   int host_edge_index = lookupEdge(morphism, 0);
   HostLabel label_e0 = getEdgeLabel(horizontal_host, host_edge_index);
   if(record_changes) pushRemarkedEdge(host_edge_index, label_e0.mark);
   changeEdgeMark(horizontal_host, host_edge_index, 1);
...
bool fillpothorizontal_makeBlueRed(MorphismPot *pot, Morphism *morphism)
{
   if(2 > horizontal_host->number_of_nodes || 1 > horizontal_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "horizontal_makeBlueRed", horizontal_host);
