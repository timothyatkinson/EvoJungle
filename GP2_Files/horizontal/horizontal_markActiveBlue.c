...
void applyhorizontal_markActiveBlue(Morphism *morphism, bool record_changes)
{
   ...
   HostLabel label_n1 = getNodeLabel(horizontal_host, host_node_index);
   if(record_changes) pushRemarkedNode(host_node_index, label_n1.mark);
   changeNodeMark(horizontal_host, host_node_index, 4);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "horizontal_markActiveBlue", horizontal_host);
