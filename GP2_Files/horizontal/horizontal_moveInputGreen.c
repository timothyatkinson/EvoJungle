...
bool horizontal_b3 = true;

static bool evaluateCondition(void)
{
   return (horizontal_b3);
}

static void evaluatePredicatehorizontal_3(Morphism *morphism)
{
   ...
   if(var_4 == var_5) horizontal_b3 = true;
   else horizontal_b3 = false;
}
...
{
   if(3 > horizontal_host->number_of_nodes || 1 > horizontal_host->number_of_edges) return false;
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(horizontal_host, start_index);

   int counter;
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      Edge *host_edge = getNthOutEdge(horizontal_host, host_node, counter);
      ...
      {
         Node *end_node = getNode(horizontal_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(horizontal_host, host_edge);
   ...
   do
   {
      ...
      if(result >= 0)
      {
         ...
         evaluatePredicatehorizontal_3(morphism);
...
void applyhorizontal_moveInputGreen(Morphism *morphism, bool record_changes)
{
   ...
   if(record_changes)
   {
      Edge *edge = getEdge(horizontal_host, host_edge_index);
   ...
   removeEdge(horizontal_host, host_edge_index);

   int source, target;
   int edge_array_size0 = horizontal_host->edges.size;
   ...
   host_edge_index = addEdge(horizontal_host, label, source, target);
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size0 == horizontal_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, horizontal_host);
}
...
static bool fillpot_n2(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < horizontal_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "horizontal_moveInputGreen", horizontal_host);
