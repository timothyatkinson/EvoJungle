...
bool init_individual_b0 = true;

static bool evaluateCondition(void)
{
   return (init_individual_b0);
}

static void evaluatePredicateinit_individual_0(Morphism *morphism)
{
   ...
   if(var_1 > 0) init_individual_b0 = true;
   else init_individual_b0 = false;
}
   ...
   for(nodes = getRootNodeList(init_individual_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateinit_individual_0(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               init_individual_b0 = true;
               break;
            }
         }
         else break;
...
void applyinit_individual_addNode(Morphism *morphism, bool record_changes)
{
   ...
   HostLabel label_n1 = getNodeLabel(init_individual_host, host_node_index);
   ...
   int node_array_size2 = init_individual_host->nodes.size;
   ...
   host_node_index = addNode(init_individual_host, 0, label);
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size2 == init_individual_host->nodes.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, init_individual_host);
}
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(init_individual_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "init_individual_addNode", init_individual_host);
