...
static void evaluatePredicateinit_individual_5(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) init_individual_b5 = true;
   else init_individual_b5 = false;
}

static void evaluatePredicateinit_individual_6(Morphism *morphism)
{
   int n0 = lookupNode(morphism, 0);
   /* If the node is not yet matched by the morphism, return. */
   if(n0 == -1) return;

   if(getOutdegree(init_individual_host, n0) < 1) init_individual_b6 = true;
...
static bool match_n0(Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < init_individual_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         evaluatePredicateinit_individual_6(morphism);
   ...
   for(host_index = 0; host_index < init_individual_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            new_assignments += result;
            /* Update global booleans for the variable's predicates. */
            evaluatePredicateinit_individual_5(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               init_individual_b5 = true;
               break;
            }
         }
         else break;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < init_individual_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "init_individual_connectOutput", init_individual_host);
