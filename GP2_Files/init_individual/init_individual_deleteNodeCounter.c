...
void applyinit_individual_deleteNodeCounter(Morphism *morphism, bool record_changes)
{
   ...
   for(count = 0; count < morphism->edges; count++)
   {
      if(record_changes)
      {
         Edge *edge = getEdge(init_individual_host, morphism->edge_map[count].host_index);
         ...
         pushRemovedEdge(edge->label, edge->source, edge->target, edge->index,
                         edge->index < init_individual_host->edges.size - 1);
      }
      removeEdge(init_individual_host, morphism->edge_map[count].host_index);
   }
   for(count = 0; count < morphism->nodes; count++)
   {
      if(record_changes)
      {
         Node *node = getNode(init_individual_host, morphism->node_map[count].host_index);
         /* A hole is created if the node is not at the right-most index of the array. */
         pushRemovedNode(node->root, node->label, node->index,
                         node->index < init_individual_host->nodes.size - 1);
      }
      removeNode(init_individual_host, morphism->node_map[count].host_index);
   }
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(init_individual_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "init_individual_deleteNodeCounter", init_individual_host);
