...
bool matchinit_individual_initNodeCounter(Morphism *morphism)
{
...
void applyinit_individual_initNodeCounter(Morphism *morphism, bool record_changes)
{
   int var_0 = getIntegerValue(morphism, 0);
   int host_node_index;
   int node_array_size1 = init_individual_host->nodes.size;
   HostLabel label;
   ...
   host_node_index = addNode(init_individual_host, 1, label);
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size1 == init_individual_host->nodes.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, init_individual_host);
}
...
bool fillpotinit_individual_initNodeCounter(MorphismPot *pot, Morphism *morphism)
{
   if(1 > init_individual_host->number_of_nodes || 0 > init_individual_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(init_individual_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "init_individual_initNodeCounter", init_individual_host);
