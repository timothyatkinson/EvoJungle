...
bool init_individual_b4 = true;

static bool evaluateCondition(void)
{
   return (init_individual_b4);
}

static void evaluatePredicateinit_individual_4(Morphism *morphism)
{
   ...
   if(var_4 < var_5 + 1) init_individual_b4 = true;
   else init_individual_b4 = false;
}
...
{
   if(2 > init_individual_host->number_of_nodes || 1 > init_individual_host->number_of_edges) return false;
   ...
   for(host_index = 0; host_index < init_individual_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateinit_individual_4(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               init_individual_b4 = true;
               break;
            }
         }
         else break;
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(init_individual_host, start_index);

   int counter;
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      Edge *host_edge = getNthOutEdge(init_individual_host, host_node, counter);
      ...
      {
         Node *end_node = getNode(init_individual_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(init_individual_host, host_edge);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "init_individual_updateDepth", init_individual_host);
