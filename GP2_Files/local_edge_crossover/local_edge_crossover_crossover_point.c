...
bool local_edge_crossover_b0 = true;

static bool evaluateCondition(void)
{
   return (local_edge_crossover_b0);
}

static void evaluatePredicatelocal_edge_crossover_0(Morphism *morphism)
{
   ...
   if(var_7 + var_8 <= var_10) local_edge_crossover_b0 = true;
   else local_edge_crossover_b0 = false;
}
...
{
   if(5 > local_edge_crossover_host->number_of_nodes || 2 > local_edge_crossover_host->number_of_edges) return false;
...
{
   RootNodes *nodes;
   for(nodes = getRootNodeList(local_edge_crossover_host); nodes != NULL; nodes = nodes->next)
   {
      Node *host_node = getNode(local_edge_crossover_host, nodes->index);
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicatelocal_edge_crossover_0(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               local_edge_crossover_b0 = true;
               break;
            }
         }
         else break;
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(local_edge_crossover_host, start_index);

   int counter;
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      Edge *host_edge = getNthOutEdge(local_edge_crossover_host, host_node, counter);
      ...
      {
         Node *end_node = getNode(local_edge_crossover_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(local_edge_crossover_host, host_edge);
...
void applylocal_edge_crossover_crossover_point(Morphism *morphism, bool record_changes)
{
   ...
   if(record_changes)
   {
      Edge *edge = getEdge(local_edge_crossover_host, host_edge_index);
      ...
      pushRemovedEdge(edge->label, edge->source, edge->target, edge->index,
                      edge->index < local_edge_crossover_host->edges.size - 1);
   }
   removeEdge(local_edge_crossover_host, host_edge_index);
   ...
   if(record_changes)
   {
      Node *node = getNode(local_edge_crossover_host, host_node_index);
      /* A hole is created if the node is not at the right-most index of the array. */
      pushRemovedNode(node->root, node->label, node->index,
                      node->index < local_edge_crossover_host->nodes.size - 1);
   }
   removeNode(local_edge_crossover_host, host_node_index);

   /* Array of host node indices indexed by RHS node index. */
   int rhs_node_map[5];

   int node_array_size4 = local_edge_crossover_host->nodes.size;
   HostLabel label;
   ...
   host_node_index = addNode(local_edge_crossover_host, 1, label);
   ...
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size4 == local_edge_crossover_host->nodes.size);
   int source, target;
   int edge_array_size0 = local_edge_crossover_host->edges.size;
   ...
   host_edge_index = addEdge(local_edge_crossover_host, label, source, target);
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size0 == local_edge_crossover_host->edges.size);
   int edge_array_size1 = local_edge_crossover_host->edges.size;
   ...
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size1 == local_edge_crossover_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, local_edge_crossover_host);
}
...
bool fillpotlocal_edge_crossover_crossover_point(MorphismPot *pot, Morphism *morphism)
{
   if(5 > local_edge_crossover_host->number_of_nodes || 2 > local_edge_crossover_host->number_of_edges) return false;
...
static bool fillpot_n3(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "local_edge_crossover_crossover_point", local_edge_crossover_host);
