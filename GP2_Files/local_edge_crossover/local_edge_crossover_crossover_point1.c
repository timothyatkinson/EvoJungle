...
bool matchlocal_edge_crossover_crossover_point1(Morphism *morphism)
{
   if(2 > local_edge_crossover_host->number_of_nodes || 1 > local_edge_crossover_host->number_of_edges) return false;
...
void applylocal_edge_crossover_crossover_point1(Morphism *morphism, bool record_changes)
{
   Assignment var_2 = getAssignment(morphism, 2);
   int host_edge_index = lookupEdge(morphism, 0);
   HostLabel label_e0 = getEdgeLabel(local_edge_crossover_host, host_edge_index);
   HostLabel label;
   ...
   else
   {
      ...
      relabelEdge(local_edge_crossover_host, host_edge_index, label);
   }
   int host_node_index = lookupNode(morphism, 0);
   Node *node0 = getNode(local_edge_crossover_host, host_node_index);
   if(!node0->root)
   {
   ...
   Node *node1 = getNode(local_edge_crossover_host, host_node_index);
   if(!node1->root)
   {
      if(record_changes) pushChangedRootNode(host_node_index);
      changeRoot(local_edge_crossover_host, host_node_index);
...
bool fillpotlocal_edge_crossover_crossover_point1(MorphismPot *pot, Morphism *morphism)
{
   if(2 > local_edge_crossover_host->number_of_nodes || 1 > local_edge_crossover_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "local_edge_crossover_crossover_point1", local_edge_crossover_host);
