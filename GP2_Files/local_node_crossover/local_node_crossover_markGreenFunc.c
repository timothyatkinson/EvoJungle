...
bool local_node_crossover_b1 = true;

static bool evaluateCondition(void)
{
   return (local_node_crossover_b1);
}

static void evaluatePredicatelocal_node_crossover_1(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) local_node_crossover_b1 = true;
   else local_node_crossover_b1 = false;
}
...
bool matchlocal_node_crossover_markGreenFunc(Morphism *morphism)
{
   if(2 > local_node_crossover_host->number_of_nodes || 1 > local_node_crossover_host->number_of_edges) return false;
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   ...
   do
   {
      ...
      if(result >= 0)
      {
         ...
         evaluatePredicatelocal_node_crossover_1(morphism);
         if(!evaluateCondition())
         {
            /* Reset the boolean variables in the predicates of this variable. */
            local_node_crossover_b1 = true;
            break;
         }
      }
      else break;
...
void applylocal_node_crossover_markGreenFunc(Morphism *morphism, bool record_changes)
{
   int host_node_index = lookupNode(morphism, 1);
   HostLabel label_n1 = getNodeLabel(local_node_crossover_host, host_node_index);
   if(record_changes) pushRemarkedNode(host_node_index, label_n1.mark);
   changeNodeMark(local_node_crossover_host, host_node_index, 2);
...
bool fillpotlocal_node_crossover_markGreenFunc(MorphismPot *pot, Morphism *morphism)
{
   if(2 > local_node_crossover_host->number_of_nodes || 1 > local_node_crossover_host->number_of_edges) return false;
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "local_node_crossover_markGreenFunc", local_node_crossover_host);
