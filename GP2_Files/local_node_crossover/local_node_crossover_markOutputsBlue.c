...
bool matchlocal_node_crossover_markOutputsBlue(Morphism *morphism)
{
   if(1 > local_node_crossover_host->number_of_nodes || 0 > local_node_crossover_host->number_of_edges) return false;
...
void applylocal_node_crossover_markOutputsBlue(Morphism *morphism, bool record_changes)
{
   int host_node_index = lookupNode(morphism, 0);
   HostLabel label_n0 = getNodeLabel(local_node_crossover_host, host_node_index);
   if(record_changes) pushRemarkedNode(host_node_index, label_n0.mark);
   changeNodeMark(local_node_crossover_host, host_node_index, 4);
...
bool fillpotlocal_node_crossover_markOutputsBlue(MorphismPot *pot, Morphism *morphism)
{
   if(1 > local_node_crossover_host->number_of_nodes || 0 > local_node_crossover_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < local_node_crossover_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "local_node_crossover_markOutputsBlue", local_node_crossover_host);
