...
static void evaluatePredicatemutate_edge_0(Morphism *morphism)
{
   ...
   if(var_7 + var_8 > var_10) mutate_edge_b0 = true;
   else mutate_edge_b0 = false;
}
...
{
   if(4 > mutate_edge_host->number_of_nodes || 1 > mutate_edge_host->number_of_edges) return false;
...
static bool fillpot_n2(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < mutate_edge_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_edge_markDepth", mutate_edge_host);
