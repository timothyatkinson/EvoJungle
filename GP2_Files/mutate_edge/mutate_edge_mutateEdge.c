...
bool mutate_edge_b1 = true;

static bool evaluateCondition(void)
{
   return (mutate_edge_b1);
}

static void evaluatePredicatemutate_edge_1(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) mutate_edge_b1 = true;
   else mutate_edge_b1 = false;
}
   ...
   for(host_index = 0; host_index < mutate_edge_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicatemutate_edge_1(morphism);
