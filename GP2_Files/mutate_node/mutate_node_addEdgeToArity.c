...
bool mutate_node_b5 = true;
bool mutate_node_b6 = true;

static bool evaluateCondition(void)
{
   return (mutate_node_b5 && mutate_node_b6);
}

static void evaluatePredicatemutate_node_5(Morphism *morphism)
{
   ...
   if(var_4 < var_5) mutate_node_b5 = true;
   else mutate_node_b5 = false;
}

static void evaluatePredicatemutate_node_6(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) mutate_node_b6 = true;
   else mutate_node_b6 = false;
}
...
{
   if(4 > mutate_node_host->number_of_nodes || 0 > mutate_node_host->number_of_edges) return false;
   ...
   for(nodes = getRootNodeList(mutate_node_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicatemutate_node_5(morphism);
   ...
   for(host_index = 0; host_index < mutate_node_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            new_assignments += result;
            /* Update global booleans for the variable's predicates. */
            evaluatePredicatemutate_node_6(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               mutate_node_b6 = true;
...
void applymutate_node_addEdgeToArity(Morphism *morphism, bool record_changes)
{
   ...
   HostLabel label_n2 = getNodeLabel(mutate_node_host, host_node_index);
   HostLabel label;
   int list_var_length0 = 0;
   ...
   else
   {
      ...
      relabelNode(mutate_node_host, host_node_index, label);
   }
   int host_edge_index;
   int source, target;
   int edge_array_size0 = mutate_node_host->edges.size;
   ...
   host_edge_index = addEdge(mutate_node_host, label, source, target);
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size0 == mutate_node_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, mutate_node_host);
}
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < mutate_node_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_node_addEdgeToArity", mutate_node_host);
