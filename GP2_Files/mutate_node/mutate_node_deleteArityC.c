...
void applymutate_node_deleteArityC(Morphism *morphism, bool record_changes)
{
   ...
   for(count = 0; count < morphism->edges; count++)
   {
      if(record_changes)
      {
         Edge *edge = getEdge(mutate_node_host, morphism->edge_map[count].host_index);
      ...
      removeEdge(mutate_node_host, morphism->edge_map[count].host_index);
   }
   for(count = 0; count < morphism->nodes; count++)
   {
      if(record_changes)
      {
         Node *node = getNode(mutate_node_host, morphism->node_map[count].host_index);
         /* A hole is created if the node is not at the right-most index of the array. */
         pushRemovedNode(node->root, node->label, node->index,
                         node->index < mutate_node_host->nodes.size - 1);
      }
      removeNode(mutate_node_host, morphism->node_map[count].host_index);
   }
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < mutate_node_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_node_deleteArityC", mutate_node_host);
