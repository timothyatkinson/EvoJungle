...
bool mutate_node_b7 = true;

static bool evaluateCondition(void)
{
   return (mutate_node_b7);
}

static void evaluatePredicatemutate_node_7(Morphism *morphism)
{
   ...
   if(var_4 > var_5) mutate_node_b7 = true;
   else mutate_node_b7 = false;
}
...
{
   if(4 > mutate_node_host->number_of_nodes || 1 > mutate_node_host->number_of_edges) return false;
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(mutate_node_host, start_index);

   int counter;
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      Edge *host_edge = getNthOutEdge(mutate_node_host, host_node, counter);
      ...
      {
         Node *end_node = getNode(mutate_node_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(mutate_node_host, host_edge);
   ...
   for(nodes = getRootNodeList(mutate_node_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicatemutate_node_7(morphism);
...
void applymutate_node_deleteEdgeToArity(Morphism *morphism, bool record_changes)
{
   ...
   if(record_changes)
   {
      Edge *edge = getEdge(mutate_node_host, host_edge_index);
      ...
      pushRemovedEdge(edge->label, edge->source, edge->target, edge->index,
                      edge->index < mutate_node_host->edges.size - 1);
   }
   removeEdge(mutate_node_host, host_edge_index);
...
static bool fillpot_n3(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(mutate_node_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_node_deleteEdgeToArity", mutate_node_host);
