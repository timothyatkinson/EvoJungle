...
bool mutate_node_b3 = true;
bool mutate_node_b4 = true;

static bool evaluateCondition(void)
{
   return (mutate_node_b3 && mutate_node_b4);
}

static void evaluatePredicatemutate_node_3(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) mutate_node_b3 = true;
   else mutate_node_b3 = false;
}

static void evaluatePredicatemutate_node_4(Morphism *morphism)
{
   ...
   if(var_7 + var_8 > var_10) mutate_node_b4 = true;
   else mutate_node_b4 = false;
}
...
{
   if(3 > mutate_node_host->number_of_nodes || 0 > mutate_node_host->number_of_edges) return false;
   ...
   for(nodes = getRootNodeList(mutate_node_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicatemutate_node_3(morphism);
         ...
         if(result >= 0)
         {
            new_assignments += result;
            /* Update global booleans for the variable's predicates. */
            evaluatePredicatemutate_node_4(morphism);
            if(!evaluateCondition())
            {
               /* Reset the boolean variables in the predicates of this variable. */
               mutate_node_b4 = true;
...
void applymutate_node_markDepth(Morphism *morphism, bool record_changes)
{
   ...
   HostLabel label_n1 = getNodeLabel(mutate_node_host, host_node_index);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < mutate_node_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_node_markDepth", mutate_node_host);
