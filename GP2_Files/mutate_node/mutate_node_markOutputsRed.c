...
{
   if(2 > mutate_node_host->number_of_nodes || 1 > mutate_node_host->number_of_edges) return false;
...
static bool match_n1(Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(mutate_node_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(host_node->indegree < 1 || host_node->outdegree < 0 ||
         ((host_node->outdegree + host_node->indegree - 0 - 1 - 0) < 0)) continue;
...
static bool match_e0(Morphism *morphism)
{
   ...
   {
      Edge *host_edge = getNthInEdge(mutate_node_host, host_node, counter);
      ...
      /* If the end node has been matched, check that the source of the
       * host edge is the image of the end node. */
      if(end_index >= 0)
      {
         if(host_edge->source != end_index) continue;
      }
      /* Otherwise, the source of the host edge should be unmatched. */
      else
      {
         Node *end_node = getNode(mutate_node_host, host_edge->source);
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "mutate_node_markOutputsRed", mutate_node_host);
