...
bool mutate_node_b2 = true;

static bool evaluateCondition(void)
{
   return (mutate_node_b2);
}

static void evaluatePredicatemutate_node_2(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) mutate_node_b2 = true;
   else mutate_node_b2 = false;
}
...
{
   if(2 > mutate_node_host->number_of_nodes || 0 > mutate_node_host->number_of_edges) return false;
   ...
   for(nodes = getRootNodeList(mutate_node_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicatemutate_node_2(morphism);
...
void applymutate_node_mutateNode(Morphism *morphism, bool record_changes)
{
   ...
   int outdegree0 = getOutdegree(mutate_node_host, node_index);
   ...
   int node_array_size2 = mutate_node_host->nodes.size;
   int list_var_length1 = 0;
   ...
   host_node_index = addNode(mutate_node_host, 1, label);
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size2 == mutate_node_host->nodes.size);
   int node_array_size3 = mutate_node_host->nodes.size;
   ...
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size3 == mutate_node_host->nodes.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, mutate_node_host);
}
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(mutate_node_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_node_mutateNode", mutate_node_host);
