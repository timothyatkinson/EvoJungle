...
{
   if(3 > mutate_node_host->number_of_nodes || 1 > mutate_node_host->number_of_edges) return false;
...
void applymutate_node_readdEdge(Morphism *morphism, bool record_changes)
{
   ...
   HostLabel label_e0 = getEdgeLabel(mutate_node_host, host_edge_index);
   HostLabel label;
   ...
   else
   {
      ...
      relabelEdge(mutate_node_host, host_edge_index, label);
   }
...
static bool fillpot_n2(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(mutate_node_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_node_readdEdge", mutate_node_host);
