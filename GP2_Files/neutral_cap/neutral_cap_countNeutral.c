...
bool neutral_cap_b0 = true;

static bool evaluateCondition(void)
{
   return (neutral_cap_b0);
}

static void evaluatePredicateneutral_cap_0(Morphism *morphism)
{
   ...
   if(!equalHostLists(array0, array1, list_length0, list_length1)) neutral_cap_b0 = true;
   else neutral_cap_b0 = false;
}
...
{
   if(2 > neutral_cap_host->number_of_nodes || 0 > neutral_cap_host->number_of_edges) return false;
   ...
   for(host_index = 0; host_index < neutral_cap_host->nodes.size; host_index++)
   {
      ...
      do
      {
         ...
         if(result >= 0)
         {
            ...
            evaluatePredicateneutral_cap_0(morphism);
...
void applyneutral_cap_countNeutral(Morphism *morphism, bool record_changes)
{
   ...
   changeNodeMark(neutral_cap_host, host_node_index, 4);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < neutral_cap_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "neutral_cap_countNeutral", neutral_cap_host);
