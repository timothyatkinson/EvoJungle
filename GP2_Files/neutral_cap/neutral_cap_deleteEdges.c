...
{
   if(2 > neutral_cap_host->number_of_nodes || 1 > neutral_cap_host->number_of_edges) return false;
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(neutral_cap_host, start_index);

   int counter;
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      Edge *host_edge = getNthOutEdge(neutral_cap_host, host_node, counter);
      ...
      {
         Node *end_node = getNode(neutral_cap_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(neutral_cap_host, host_edge);
...
void applyneutral_cap_deleteEdges(Morphism *morphism, bool record_changes)
{
   ...
   if(record_changes)
   {
      Edge *edge = getEdge(neutral_cap_host, host_edge_index);
      ...
      pushRemovedEdge(edge->label, edge->source, edge->target, edge->index,
                      edge->index < neutral_cap_host->edges.size - 1);
   }
   removeEdge(neutral_cap_host, host_edge_index);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "neutral_cap_deleteEdges", neutral_cap_host);
