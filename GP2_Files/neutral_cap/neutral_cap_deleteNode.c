...
void applyneutral_cap_deleteNode(Morphism *morphism, bool record_changes)
{
   ...
   if(record_changes)
   {
      Node *node = getNode(neutral_cap_host, host_node_index);
      /* A hole is created if the node is not at the right-most index of the array. */
      pushRemovedNode(node->root, node->label, node->index,
                      node->index < neutral_cap_host->nodes.size - 1);
   }
   removeNode(neutral_cap_host, host_node_index);
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < neutral_cap_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "neutral_cap_deleteNode", neutral_cap_host);
