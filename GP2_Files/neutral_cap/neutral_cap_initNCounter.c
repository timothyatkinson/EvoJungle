...
bool matchneutral_cap_initNCounter(Morphism *morphism)
{
...
void applyneutral_cap_initNCounter(Morphism *morphism, bool record_changes)
{
   int host_node_index;
   int node_array_size1 = neutral_cap_host->nodes.size;
   HostLabel label;
   ...
   host_node_index = addNode(neutral_cap_host, 1, label);
   /* If the node array size has not increased after the node addition, then
      the node was added to a hole in the array. */
   if(record_changes)
      pushAddedNode(host_node_index, node_array_size1 == neutral_cap_host->nodes.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, neutral_cap_host);
}
...
bool fillpotneutral_cap_initNCounter(MorphismPot *pot, Morphism *morphism)
{
   if(1 > neutral_cap_host->number_of_nodes || 0 > neutral_cap_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(neutral_cap_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "neutral_cap_initNCounter", neutral_cap_host);
