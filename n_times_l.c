...
int main(void){
  ...
  for(int i = 4; i < 21; i++){
    ...
    for(int j = 0; j < runs; j++){
      printf("Run %d\n", j);
      double test_mse = n_times_l(train_d, validation_d, test_d, fset, 5, 1, 40, 40, 2.0, 4990);
      printf("\nFinal test MSE = %.10e\n", test_mse);
      res[2 * i][j] = test_mse;
    ...
    for(int j = 0; j < runs; j++){
      printf("Run %d\n", j);
      double test_mse = n_times_l(train_d_n, validation_d_n, test_d_n, fset, 5, 1, 40, 40, 2.0, 4990);
      printf("\nFinal test MSE = %.10e\n", test_mse);
